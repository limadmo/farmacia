/**
 * Controller de Auditoria Inteligente - Sistema de Farmácia
 * 
 * Gerencia endpoints REST para auditoria de vendas de medicamentos controlados
 * com recursos inteligentes de paginação, densidade temporal e marcos importantes.
...
export class AuditoriaController {
  ...
  async listarVendasControladas(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Novo filtro para receitas
      const { comReceita } = req.query;
      if (comReceita === 'true') {
        filtro.comReceita = true;
      }

      const resultado = await this.auditoriaService.listarVendasControladas(filtro);

      this.logger.info(`🔍 Auditoria consultada por ${req.user?.nome}: ${resultado.total} vendas controladas (página ${filtro.page})`);
   ...
   */
  async obterDensidadeTemporal(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const { dataInicio, dataFim } = req.query;
      ...
      const densidade = await this.auditoriaService.obterDensidadeTemporal(filtro);

      this.logger.info(`📊 Densidade temporal consultada por ${req.user?.nome}`);

      res.json(densidade);
    } catch (error) {
      this.logger.error('Erro ao obter densidade temporal:', error);
      next(error);
    }
  }
   ...
   */
  async obterMarcosImportantes(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const { dataInicio, dataFim } = req.query;
      ...
      const marcos = await this.auditoriaService.obterMarcosImportantes(filtro);

      this.logger.info(`🎯 Marcos importantes consultados por ${req.user?.nome}`);
   ...
   */
  async obterSugestoesFiltros(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const sugestoes = await this.auditoriaService.obterSugestoesFiltros();

      this.logger.info(`🧠 Sugestões filtros consultadas por ${req.user?.nome}`);

      res.json(sugestoes);
    } catch (error) {
      this.logger.error('Erro ao obter sugestões de filtros:', error);
      next(error);
    }
  }
}