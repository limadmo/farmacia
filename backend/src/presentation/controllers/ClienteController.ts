...
import { ClienteService, ListClientesParams } from '@/application/services/ClienteService';
...
export class ClienteController {
  ...
  async listar(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Se paginated=false explícito, usar o método antigo para compatibilidade
      if (paginated === 'false') {
      ...
      // Usar paginação por padrão - sempre garantir page e limit
      const params: ListClientesParams = {
        page: page ? parseInt(page as string, 10) : 1,
        limit: limit ? parseInt(limit as string, 10) : 20,
        search: search as string,
        ativo: ativo === 'true' ? true : ativo === 'false' ? false : undefined,
        creditoHabilitado: creditoHabilitado === 'true' ? true : creditoHabilitado === 'false' ? false : undefined,
        tipoDocumento: tipoDocumento as TipoDocumento
      };

      // Remover apenas parâmetros opcionais undefined (não page e limit)
      const cleanParams: any = { ...params };
      ['search', 'ativo', 'creditoHabilitado', 'tipoDocumento'].forEach(key => {
        if (cleanParams[key] === undefined) {
          delete cleanParams[key];
        }
      });

      const resultado = await this.clienteService.listarClientes(cleanParams);
      res.json(resultado);
    } catch (error) {
      next(error);
    }
