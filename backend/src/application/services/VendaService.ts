...
export class VendaService {
  ...
  async criarVenda(data: CriarVendaData, usuarioId: string): Promise<VendaComItens> {
    ...
    try {
      ...
      for (const item of itensProcessados) {
        ...
        // Processar item considerando controle de lote opcional
        const produto = produtos.find(p => p.id === item.produtoId);
        if (produto) {
          await this.processarItemComLote(item, produto, venda.id, usuarioId, tx);
        }
      }
  ...
  /**
   * Processa item de venda considerando controle de lote opcional
   */
  private async processarItemComLote(
    item: any,
    produto: any,
    vendaId: string,
    usuarioId: string,
    ...
    // Verificar se produto requer controle de lote obrigatório
    if (produto.loteObrigatorio) {
      // Para produtos com lote obrigatório, a seleção deve ter sido feita no frontend
      // Por enquanto, vamos manter a lógica atual (estoque geral)
      // TODO: Implementar seleção explícita de lotes no frontend
      this.logger.info(`⚠️ Produto ${produto.nome} requer seleção manual de lote (não implementado ainda no frontend)`);
      return this.processarItemSemLote(item, produto, vendaId, usuarioId, tx);
    } else {
      // Para produtos com lote opcional, usar lógica FEFO automática
      return this.processarItemComLoteAutomatico(item, produto, vendaId, usuarioId, tx);
    }
  }

  /**
   * Processa item sem controle de lote (método atual)
   */
  private async processarItemSemLote(
    item: any,
    produto: any,
    vendaId: string,
    usuarioId: string,
    ...
    if (!produtoAtual) {
      throw new Error(`Produto ${item.produtoId} não encontrado durante a transação`);
    }

    // Validação rigorosa: garantir que o estoque não ficará negativo
    if (produtoAtual.estoque < item.quantidade) {
      throw new Error(`Estoque insuficiente para ${produto.nome}. Disponível: ${produtoAtual.estoque}, Solicitado: ${item.quantidade}`);
    }
  ...
  /**
   * Processa item usando lógica FEFO automática de lotes
   */
  private async processarItemComLoteAutomatico(
    item: any,
    produto: any,
    vendaId: string,
    usuarioId: string,
    ...
    try {
      // Usar EstoqueService para obter lotes FEFO
      const lotesSelecionados = await this.estoqueService.obterLotesAutomaticosFEFO(
        item.produtoId, 
        item.quantidade
      );

      if (lotesSelecionados.length === 0) {
        // Fallback para método sem lote se não houver lotes disponíveis
        this.logger.warn(`⚠️ Nenhum lote disponível para ${produto.nome}, usando controle de estoque geral`);
        return this.processarItemSemLote(item, produto, vendaId, usuarioId, tx);
      }

      let quantidadeRestante = item.quantidade;
      let estoqueDecrementado = 0;

      // Processar cada lote selecionado
      for (const lote of lotesSelecionados) {
        if (quantidadeRestante <= 0) break;

        const disponivel = lote.quantidadeAtual - lote.quantidadeReservada;
        const quantidadeDoLote = Math.min(disponivel, quantidadeRestante);

        if (quantidadeDoLote <= 0) continue;

        // Atualizar quantidade do lote
        await tx.lote.update({
          where: { id: lote.id },
          data: { 
            quantidadeAtual: { decrement: quantidadeDoLote }
          }
        });
        ...
        // Criar registro ItemVendaLote para rastreabilidade
        const itemVendaId = await tx.itemVenda.findFirst({
          where: { 
            vendaId: vendaId,
            produtoId: item.produtoId 
          },
        ...
        if (itemVendaId) {
          await tx.itemVendaLote.create({
            data: {
              itemVendaId: itemVendaId.id,
              loteId: lote.id,
              quantidade: quantidadeDoLote
            }
          });
        ...
        estoqueDecrementado += quantidadeDoLote;

        this.logger.info(`🎯 Lote processado: ${lote.numeroLote} (${quantidadeDoLote} unidades) - Produto: ${produto.nome}`);
      }

      // Atualizar estoque geral do produto
      if (estoqueDecrementado > 0) {
        await tx.produto.update({
          where: { id: item.produtoId },
          data: { estoque: { decrement: estoqueDecrementado } }
        });

        // Registrar movimentação geral de estoque
        await tx.movimentacaoEstoque.create({
          data: {
            produtoId: item.produtoId,
            tipo: TipoMovimentacao.SAIDA,
            quantidade: estoqueDecrementado,
            motivo: `Venda #${vendaId} (FEFO automático)`,
            usuarioId
          }
        });
      }

      // Verificar se conseguiu atender toda a quantidade
      if (quantidadeRestante > 0) {
        this.logger.warn(`⚠️ Quantidade não atendida: ${quantidadeRestante} de ${produto.nome}`);
        throw new Error(`Não foi possível atender toda a quantidade solicitada de ${produto.nome}. Faltaram ${quantidadeRestante} unidades`);
      }

    } catch (error) {
      this.logger.error(`Erro ao processar item com lote automático: ${error}`);
      // Em caso de erro, fallback para método sem lote
      this.logger.info(`📋 Fallback: usando controle de estoque geral para ${produto.nome}`);
      return this.processarItemSemLote(item, produto, vendaId, usuarioId, tx);
    }
  }
