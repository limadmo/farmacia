...
export class ProdutoService {
    ...
    try {
      ...
      const novoProduto = await this.prisma.produto.create({
        data: {
          ...
          loteObrigatorio: data.loteObrigatorio ?? false,
  ...
  async atualizarProduto(id: string, data: UpdateProdutoData): Promise<Produto> {
    try {
      // Debug: log dos dados recebidos no service
      this.logger.info(`🔧 Service recebeu para atualização:`, data);
      this.logger.info(`🔧 loteObrigatorio no service: ${data.loteObrigatorio} (tipo: ${typeof data.loteObrigatorio})`);
      ...
      // Construir dados de update usando valores processados
      const updateData = {
        ...
        retencaoReceita: dadosProcessados.retencaoReceita,
        // Usar o valor processado de loteObrigatorio ao invés da lógica duplicada
        loteObrigatorio: dadosProcessados.loteObrigatorio,
      ...
      // Debug: log dos dados processados e finais para o Prisma
      this.logger.info(`🔧 Dados processados:`, dadosProcessados);
      this.logger.info(`🎯 Dados finais para o Prisma:`, updateData);
      this.logger.info(`🎯 loteObrigatorio processado: ${dadosProcessados.loteObrigatorio} - será salvo: ${updateData.loteObrigatorio}`);
   ...
   * Mantém todos os campos necessários incluindo loteObrigatorio
   */
  private processarMedicamentoControlado(data: CreateProdutoData) {
    ...
    // Preservar o valor de loteObrigatorio dos dados originais
    let loteObrigatorio = data.loteObrigatorio ?? false;
