  ...
  useEffect(() => {
    ...
    const carregarVenda = async () => {
      try {
        ...
        // Não mostrar erro para venda não encontrada (404) - é esperado
        if (error.response?.status === 404) {
          // Venda não encontrada - será tratado pela UI
          return;
        }
        ...
        if (error.response?.status >= 500 || !error.response) {
          ...
          toast.error('Acesso negado para visualizar esta venda');
        }
      } finally {
        setLoading(false);
      }
