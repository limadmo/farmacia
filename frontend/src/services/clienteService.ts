...
export class ClienteService {
   ...
   */
  async listarClientes(filters?: ClienteFilters & {
    page?: number;
    limit?: number;
    paginated?: boolean;
  }): Promise<ClienteListResponse | ClienteResponse[]> {
    try {
      const params = new URLSearchParams();
      
      // Por padrão, usar paginação
      ...
      if (filters?.creditoHabilitado !== undefined) {
        params.append('creditoHabilitado', filters.creditoHabilitado.toString());
      }
      ...
      const response = usePagination 
        ? await api.get<ClienteListResponse>(url)
        : await api.get<ClienteResponse[]>(url);
