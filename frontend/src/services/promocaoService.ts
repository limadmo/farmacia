...
class PromocaoService {
   ...
   */
  async buscarPromocoesAplicaveis(produto: { id: string; laboratorio?: string }, loteId?: string): Promise<Promocao[]> {
    try {
      const queryParams = new URLSearchParams();
      queryParams.append('produtoId', produto.id);
      if (produto.laboratorio) {
        queryParams.append('laboratorio', produto.laboratorio);
      }
      if (loteId) {
        queryParams.append('loteId', loteId);
      }

      const response = await api.get<Promocao[]>(`/promocoes/aplicaveis?${queryParams}`);
      return response.data;
    } catch (error) {
  ...
  /**
   * Lista lotes disponíveis para aplicação de promoção em um produto
   */
  async listarLotesDisponiveis(produtoId: string): Promise<LotePromocaoSelecionado[]> {
    try {
      const response = await api.get<{ lotes: LotePromocaoSelecionado[] }>(`/promocoes/lotes/${produtoId}`);
